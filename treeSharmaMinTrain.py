...
# read in lake characteristics data published by Sharma et al.     
...
#create geodataframes to spatially merge lake characteristic data with temperature data
...
#drop columns not used to train model and clean data
gdt = gdt.drop(['lat', 'lon', 'Longitude_dd', 'Latitude_dd', 'temp_range', 'tmx', 'index_right', 'geometry'], axis = 1)
dt = pd.DataFrame(gdt)
...
#save the model
joblib.dump(dt, 'treeSharmaMinTrain.sav')
...
graph.write_png('treeSharmaMinTrain.png')
